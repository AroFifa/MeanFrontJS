...
export class CommissionComponent extends ShareComponent {
    ...
    displayedColumns: string[] = ['commissionRate', 'creationDate'];
    ...
    ngOnInit() {
        ...
        this.commissionRateInput = new FormControl(
            this.commission.commissionRate,
            [Validators.required, Validators.min(0), Validators.max(100)],
        );
        this.commissionHistory =
            this.route.snapshot.data['initialData'][1].data;

        this.dataSource = new MatTableDataSource<any>(
            this.commissionHistory.items,
        );
    ...
    syncData() {
        ...
        this._commissionService.get().subscribe((item) => {
    ...
    updateCommission(event: any) {
        if (
            this.commissionRateInput.valid &&
            event.target.value != this.commission.commissionRate
        ) {
            this._matDialog
